...
--子列映射
local notation "φ" => Converage_Subseq.φ (Subseq fullrank₁ fullrank₂)

--子列收敛的点
local notation "x₁''" => Converage_Subseq.x₁'' (Subseq fullrank₁ fullrank₂)
local notation "x₂''" => Converage_Subseq.x₂'' (Subseq fullrank₁ fullrank₂)
local notation "y''"  => Converage_Subseq.y'' (Subseq fullrank₁ fullrank₂)
...
atTop (𝓝 x₁'' ×ˢ 𝓝 x₂'' ×ˢ 𝓝 y''):=by
   have := (Subseq fullrank₁ fullrank₂).hconverage
   rw[admm_nhds_prod_eq] at this
   exact this

lemma x₁_subseq_converage : Tendsto (fun n => (x₁ (φ n)))  atTop (𝓝 x₁'') :=
   (Filter.tendsto_prod_iff'.1 (hconverage fullrank₁ fullrank₂)).1

lemma x₂_subseq_converage : Tendsto (fun n => (x₂ (φ n)))  atTop (𝓝 x₂'') :=
   (Filter.tendsto_prod_iff'.1 (Filter.tendsto_prod_iff'.1 (hconverage fullrank₁ fullrank₂)).2).1

lemma y_subseq_converage  : Tendsto (fun n => (y (φ n)))  atTop (𝓝 y'') :=
   (Filter.tendsto_prod_iff'.1 (Filter.tendsto_prod_iff'.1 (hconverage fullrank₁ fullrank₂)).2).2
...
-- byf
-- ‖A₁ (e₁ n) + A₂ (e₂ n)‖ → 0
lemma converage_zero₁ : Tendsto (fun n => ‖A₁ (e₁ n) + A₂ (e₂ n)‖)  atTop (𝓝 0) := by
   have : Tendsto (fun n => ‖A₁ (e₁ n) + A₂ (e₂ n)‖ ^ 2)  atTop (𝓝 0) := by apply Summable.tendsto_atTop_zero Summable₁
   apply square_converge_zero₁ this

-- byf
-- ‖A₁ (x₁ n) + A₂ (x₂ n) - b‖ → 0
lemma converage_zero₂ : Tendsto (fun n => ‖A₁ (x₁ n) + A₂ (x₂ n) - b‖)  atTop (𝓝 0) := by
...
lemma converage_zero₃ : Tendsto (fun n => ‖A₂ (x₂ (n + 1) - x₂ n)‖)  atTop (𝓝 0) := by
...
lemma Seq_converage_zero₁ : Tendsto (fun n => A₁ (e₁ n) + A₂ (e₂ n))  atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.2 converage_zero₁

-- A₁ (x₁ n) + A₂ (x₂ n) - b → 0
lemma Seq_converage_zero₂ : Tendsto (fun n => A₁ (x₁ n) + A₂ (x₂ n) - b)  atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.2 converage_zero₂

-- A₂ (x₂ (n + 1) - x₂ n) → 0
lemma Seq_converage_zero₃ : Tendsto (fun n => A₂ (x₂ (n + 1) - x₂ n))  atTop (𝓝 0) := by
   apply tendsto_zero_iff_norm_tendsto_zero.2 converage_zero₃

-- byf
-- A₁ (e₁ (φ₁ n)) + A₂ (e₂ (φ₁ n)) → 0
lemma sub_Seq_converage_zero₁ : Tendsto (fun n => A₁ (e₁ (φ₁ n)) + A₂ (e₂ (φ₁ n)))
atTop (𝓝 0) := by
   apply Filter.tendsto_iff_seq_tendsto.1 Seq_converage_zero₁
...
-- byf
-- A₁ (x₁ (φ₁ n)) + A₂ (x₂ (φ₁ n)) - b → 0
lemma sub_Seq_converage_zero₂ : Tendsto (fun n => A₁ (x₁ (φ₁ n)) + A₂ (x₂ (φ₁ n)) - b) atTop (𝓝 0) := by
   apply Filter.tendsto_iff_seq_tendsto.1 Seq_converage_zero₂
   apply StrictMono.tendsto_atTop
   have : (fun n => (Int.toNat (φ₁ n))) = (fun n => (φ (n+1))) := by
      ext n; rw [φ₁_equ fullrank₁ fullrank₂ n]; simp
...
-- byf
-- A₂ (x₂ ((φ₁ n) + 1) - x₂ (φ₁ n)) → 0
lemma sub_Seq_converage_zero₃ : Tendsto (fun n => A₂ (x₂ ((φ₁ n) + 1) - x₂ (φ₁ n))) atTop (𝓝 0) := by
   apply Filter.tendsto_iff_seq_tendsto.1 Seq_converage_zero₃
   apply StrictMono.tendsto_atTop
   have : (fun n => (Int.toNat (φ₁ n))) = (fun n => (φ (n+1))) := by
      ext n; rw [φ₁_equ fullrank₁ fullrank₂ n]; simp
...
-- A₂ (x₂ (φ₁ n) - x₂ ((φ₁ n) - 1)) → 0
lemma sub_Seq_converage_zero₃' : Tendsto (fun n => A₂ (x₂ (φ₁ n) - x₂ ((φ₁ n) - 1))) atTop (𝓝 0) := by
   apply Filter.tendsto_iff_seq_tendsto.1 Seq_converage_zero₃
   apply StrictMono.tendsto_atTop
...
lemma const_smul_subseq₁: Tendsto (fun n => (( 1 - τ) * ρ ) • (A₁ (e₁ (φ₁ n)) + A₂ (e₂ (φ₁ n)))) atTop (𝓝 0) := by
   rw [← smul_zero (( 1 - τ) * ρ )]
   apply Filter.Tendsto.const_smul (sub_Seq_converage_zero₁ fullrank₁ fullrank₂) (( 1 - τ) * ρ)
...
lemma const_smul_subseq₂ : Tendsto (fun n => ρ • A₂ (x₂ ((φ₁ n) - 1) - x₂ (φ₁ n))) atTop (𝓝 0) := by
   ...
   rw [this, ← smul_zero (-ρ)]
   apply Filter.Tendsto.const_smul (sub_Seq_converage_zero₃' fullrank₁ fullrank₂)

-- byf
-- u (φ₁ n) converges to (- A₁† y'')
lemma u_subseq_converage : Tendsto (fun n => u (φ₁ n)) atTop (𝓝 (- A₁† y'')) := by
   ...
   have : Tendsto (fun n => (y (φ₁ n)) + (( 1 - τ) * ρ ) • (A₁ (e₁ (φ₁ n))
      ...
      rw [← add_zero y'']
      apply Filter.Tendsto.add (y_subseq_converage' fullrank₁ fullrank₂) (const_smul_subseq₁ fullrank₁ fullrank₂)
...
lemma v_subseq_converage : Tendsto (fun n => v (φ₁ n)) atTop (𝓝 (- A₂† y'')) := by
...
lemma A₁'y_inthesubgradient : - A₁† y'' ∈ SubderivAt f₁ x₁'':=
   ...
   (fun n => x₁ (φ₁ n)) (x₁_subseq_converage' fullrank₁ fullrank₂)
   (fun n => u  (φ₁ n)) (u_subseq_converage   fullrank₁ fullrank₂)
   admm.lscf₁ (fun n => u_inthesubgradient (φ₁ n))

lemma A₂'y_inthesubgradient : - A₂† y'' ∈ SubderivAt f₂ x₂'':=
   Image_subgradient_closed f₂ x₂'' (- A₂† y'')
   (fun n => x₂ (φ₁ n)) (x₂_subseq_converage' fullrank₁ fullrank₂)
   (fun n => v  (φ₁ n)) (v_subseq_converage   fullrank₁ fullrank₂)
...
Tendsto (fun n => A₁ (x₁ (φ n))) atTop (𝓝 (A₁ x₁'')) := by
   ...
   apply tendsto_iff_seq_tendsto.1 (x₁_subseq_converage fullrank₁ fullrank₂)
   apply StrictMono.tendsto_atTop
   apply strictMono_id
...
lemma subconverage_zero₂ : Tendsto (fun n =>  ‖A₁ (x₁ (φ n)) + A₂ (x₂ (φ n)) - b‖)  atTop (𝓝 0)
:= by
   apply tendsto_iff_seq_tendsto.1 converage_zero₂
   apply StrictMono.tendsto_atTop
   apply hphi_StrictMono
...
lemma e₁_subseq_converage_zero : Tendsto (e₁ ∘ φ) atTop (𝓝 0) := by
   have h₁: Tendsto (fun n => (x₁ ∘ φ) n - x₁'') atTop (𝓝 0) := by
      apply tendsto_sub_nhds_zero_iff.2; apply x₁_subseq_converage
...
lemma ey_subseq_converage_zero : Tendsto (ey ∘ φ) atTop (𝓝 0) := by
   have h₁: Tendsto (fun n => (y ∘ φ) n - y'') atTop (𝓝 0) := by
      apply tendsto_sub_nhds_zero_iff.2; apply y_subseq_converage
...
lemma A₂e₂_pow_converage_zero : Tendsto (fun n => ‖A₂ (e₂ n)‖^2) atTop (𝓝 0) := by
   apply squeeze_zero_norm; apply A₂e₂_le_Φ'
   have h1 : Tendsto (fun n ↦ (1 / ρ) * Φ n) atTop (𝓝 (1 / ρ * 0)) := by
      apply Filter.Tendsto.const_mul (1 / ρ) (Φ_converage_zero fullrank₁ fullrank₂)
...
lemma A₂e₂_converage_zero : Tendsto (fun n => ‖A₂ (e₂ n)‖) atTop (𝓝 0) := by
   have : Tendsto (fun n => √((‖A₂ (e₂ n)‖)^2))  atTop (𝓝 √0) := by
      apply Tendsto.sqrt (A₂e₂_pow_converage_zero fullrank₁ fullrank₂)
...
lemma A₁e₁_converage_zero : Tendsto (fun n => ‖A₁ (e₁ n)‖) atTop (𝓝 0) := by
   ...
   have h'' : Tendsto (fun n => ‖A₁ (e₁ n) + A₂ (e₂ n)‖ + ‖A₂ (e₂ n)‖) atTop (𝓝 (0 + 0)) := by
      apply Tendsto.add (converage_zero₁) (A₂e₂_converage_zero fullrank₁ fullrank₂)
...
lemma A₁e₁_converage_zero' : Tendsto (fun n => ‖(A₁ ∘ e₁) n‖) atTop (𝓝 0) := by
   ...
   rw [← this]
   apply A₁e₁_converage_zero

--dyx xzx
lemma CA₁e₁_converage_zero (C : ℝ) : Tendsto (fun n => C * ‖A₁ (e₁ n)‖) atTop (𝓝 0) := by
   have : Tendsto (fun n => ‖A₁ (e₁ n)‖) atTop (𝓝 0) := by apply A₁e₁_converage_zero
...
lemma e₁_converage_zero : Tendsto e₁ atTop (𝓝 0) := by
   have : ∃ C > 0, ∀ n : ℕ, ‖e₁ n‖ ≤ C * ‖A₁ (e₁ n)‖ := open_mapping_e₁ fullrank₁
   obtain ⟨C, _, hC⟩ := this
   apply squeeze_zero_norm; intro n; exact hC n; apply CA₁e₁_converage_zero
...
lemma x₁_converage : Tendsto x₁ atTop (𝓝 x₁'') := by
   ...
   have h := e₁_converage_zero fullrank₁ fullrank₂
...
lemma y_converage : Tendsto y atTop (𝓝 y'') := by
   ...
   have h := ey_converage_zero fullrank₁ fullrank₂
   rw[this , tendsto_sub_nhds_zero_iff] at h
   exact h
...
∧ ( Tendsto x₁ atTop (𝓝 _x₁)∧ Tendsto x₂ atTop (𝓝 _x₂)∧ Tendsto y atTop (𝓝 _y)) :=
   ...
   x₁_converage fullrank₁ fullrank₂ ,x₂_converage fullrank₁ fullrank₂,
   y_converage fullrank₁ fullrank₂⟩
